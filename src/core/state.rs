...
mod tests {
    ...
    fn test_bookmark_operations() {
        ...
        manager.set_bookmark("users".to_string(), Value::String("2024-01-01".to_string()));
        assert!(manager.is_dirty());
        assert!(manager.get_bookmark("users").is_some());
    }
    ...
    fn test_global_state_operations() {
        ...
        manager.set_global("last_run".to_string(), Value::String("2024-01-01".to_string()));
        assert!(manager.is_dirty());
        assert_eq!(
            manager.get_global("last_run"),
    ...
    fn test_save_and_load() {
...

        {
            ...
            manager.set_bookmark("stream1".to_string(), Value::Number(100.into()));
            manager.save().unwrap();
        }

        {
            let mut manager = StateManager::new(&path);
            manager.load().unwrap();
            let bookmark = manager.get_bookmark("stream1").unwrap();
            assert_eq!(bookmark.value, Value::Number(100.into()));
        }
    }
    ...
    fn test_merge_states() {
        ...
        other_state.bookmarks.insert(
            "stream2".to_string(),
            Bookmark {
                value: Value::Number(200.into()),
                timestamp: Utc::now(),
                metadata: HashMap::new(),
            },
        );

        manager.merge(other_state).unwrap();
        assert!(manager.get_bookmark("stream1").is_some());
        assert!(manager.get_bookmark("stream2").is_some());
    }
}
